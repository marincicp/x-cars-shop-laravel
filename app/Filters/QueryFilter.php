...
class QueryFilter
{
    ...
    {
        $query->when($request->filled('state_id'), fn ($q) => $q->join('cities', 'cities.id', 'cars.city_id')->join('states', 'states.id', 'cities.state_id')->where('states.id', $request->input('state_id'))->addSelect('states.name as state_name'));
    }
